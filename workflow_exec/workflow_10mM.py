...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 28 10:01:11 2025
...
data_10mM = genfromtxt(os.path.join('data_10mM.csv'), delimiter=',')
data_t_10mM = data_10mM[1:, 0]

#Data Aux
data_aux_10mM=[torch.tensor([14.19223119,9.003971998,0.06713709716,0.9875,0.2273,9.05]),
              torch.tensor([0.9004836 , 10.05402847,  1.05713568,  1.06013931,  0.24415099,10.08267574])]

#Experimetal case
observables = ["GLC","ACE_env","X"]
variable_data = {"GLC": data_10mM[1:, 3], "ACE_env": data_10mM[1:, 1], "X":data_10mM[1:, 2]}
...
def acetate_overflow_model(
    ...
    y0 =[14.19223119,9.003971998,0.06713709716,0.9875,0.2273,9.05]
...
R2_scores_train_data=[r2_score(data_10mM.T[3,1:],GLC_pred.detach().numpy()),r2_score(data_10mM.T[1,1:],ACE_env_pred.detach().numpy()),
           r2_score(data_10mM.T[2,1:],X_pred.detach().numpy())]
...
R2_scores_gene_vs_data=[r2_score(data_10mM.T[3,1:],GLC_gene),r2_score(data_10mM.T[1,1:],ACE_env_gene),
           r2_score(data_10mM.T[2,1:],X_gene)]
...
axs[0,0].plot(data_t_10mM, data_10mM[1:, 3], 'o', label='GLC_data', color='b')
...
axs[0,1].plot(data_t_10mM, data_10mM[1:, 1], 'o', label='ACE_env_data', color='g')
...
axs[0,2].plot(data_t_10mM, data_10mM[1:, 2], 'o',label='X_data', color='r')
...
# initial conditions
y_1_0 = [14.19223119,9.003971998,0.06713709716,0.9875,0.2273,9.05] #10mM
...
axs[0,0].plot(data_10mM[1:, 0], data_10mM[1:, 3], 'o', label='GLC_data', color='b')
axs[0,0].set_title('GLC 1mM')
axs[0,0].set_xlabel('t (h)')
axs[0,0].set_ylabel('GLC (mM)')
axs[0,0].legend()
...
axs[0,1].plot(data_10mM[1:, 0], data_10mM[1:, 1], 'o', label='ACE_env_data', color='g')
axs[0,1].set_title('ACE_env 1mM')
axs[0,1].set_xlabel('t (h)')
axs[0,1].set_ylabel('ACE_env (mM)')
axs[0,1].legend()
...
axs[0,2].plot(data_10mM[1:, 0], data_10mM[1:, 2], 'o',label='X_data', color='r')
axs[0,2].set_title('X 1mM')
axs[0,2].set_xlabel('t (h)')
axs[0,2].set_ylabel('X (g_DW/L)')
axs[0,2].legend()
...
## R2 scores
R2_scores_vs_data=[r2_score(data_10mM.T[3,1:],GLC_learned),r2_score(data_10mM.T[1,1:],ACE_env_learned),
           r2_score(data_10mM.T[2,1:],X_learned)]
