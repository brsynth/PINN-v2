...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 29 14:45:40 2025

@author: lucie-garance
"""
## Version of PINN with only two losses
...
class Pinn(nn.Module):
    ...
    class NeuralNet(nn.Module): # input: [[t1], [t2]...[t100]] batch of timesteps
        ...
        def __init__(self,net_hidden, activation_function, output_size):
            ...
            layers = [nn.Linear(1, 20), activation_function]
    ...
    def train(self, n_epochs):
        ...
        for epoch in tqdm(range(n_epochs), desc="Training the neural network", ncols=150):
            ...
            ##Another version of SoftAdapt including an increase of weights at the end of training
                ...
                if epoch<self.soft_adapt_warming:
                    ...
                    self.residual_weights=[[self.increment if self.residual_weights[j] else 1
        ...
        if epoch >= self.soft_adapt_t:
            ...
            if self.soft_adapt_by_type:
                if self.soft_adapt_normalize:
                    ...
                    if np.linalg.norm(x[self.nb_res:])<self.soft_adapt_eps:
                        weight_var= np.ones_like(x[self.nb_res:]) / x[self.nb_res:].shape[0] 
                    else:
                        x[self.nb_res:]=x[self.nb_res:]/ \
                                    np.linalg.norm(x[self.nb_res:])
                        weight_var=softmax(x[self.nb_res:]*beta)
                        
                return weight_res, weight_var 
    ...
    def wang(self,
        ...
        return np.array(residual_method_weights), np.array(variable_method_weights)
